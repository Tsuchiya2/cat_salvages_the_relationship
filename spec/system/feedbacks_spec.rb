  ...
  describe 'フィードバックを投稿' do
    context '正常系テスト' do
      it 'トップページからフィードバックへ遷移し、投稿が成功し、トップページに戻ってくる' do
        visit root_path
        click_on 'フィードバック'
        expect(page).to have_field('feedback[text]', wait: 5)
        fill_in 'feedback[text]', with: feedback.text
        click_on '🐾 送信 🐾'
        expect(page).to have_content('フィードバックありがとうございます！', wait: 5)
    ...
    context '異常系テスト' do
      it '空の投稿を行うと、フラッシュメッセージが表示される' do
        visit new_feedback_path
        expect(page).to have_button('🐾 送信 🐾', wait: 5)
        click_on '🐾 送信 🐾'
        expect(page).to have_content('30〜500文字でお願いします！🐾', wait: 5)
